...
export const extractFileBlob = async (attachments: attachmentsType[], type: string,) => {
...
export const extractFileBlobDynamic = async ({ attachments, type, initPath, completePath }: ExtractBlobType) => {
    ...
    const test = async () => {
        ...
        const map1 = await Promise.all(attachments.map(async (item: any) => await fetch(fileImport({ type: "preview", fileName: `${completePath}/${item.path}` ?? `${type}/${initPath}/${item.path}` }),
            {
                mode: 'cors',
                headers: {
                    'Access-Control-Allow-Origin': '*'
