...
import TicketAssignment from '@/app/configurations/ticketTypes/forms/assignments';
import TicketSubmissionDepartment from '@/app/configurations/ticketTypes/forms/submissionDepartment';
import Permissibles from './permissibles';
import { PropForm } from '@/types/forms/propHookForm';
import TicketServiceDepartment from './serviceDepartment';
...
const TicketTypeFormComp = ({ code, defaults }: Props) => {
  ...
  const hookFormProp = useForm<TicketTypeForm>({
    //@ts-ignore
    defaultValues: defaults ?? {},
  ...
  const onSubmit = async (values: TicketTypeForm) => {
    ...
    values.assignments = extractObjectId(values.assignments)
    values.submissionDepartment = extractObjectId(values.submissionDepartment)
    values.serviceDepartment = _.get(values, "serviceDepartment._id")
    values.addWorkWOApprovers = values.addWorkWOApprovers
    values.addWorkWOAssignedP = values.addWorkWOAssignedP
  ...
  useEffect(() => {
    if (code) {
      const typeName: string = _.get(defaults, "name");
  ...
  return (
    <div>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <TicketAssignment register={register} errors={errors} control={control} setValue={setValue} />
        <TicketSubmissionDepartment register={register} errors={errors} control={control} setValue={setValue} />
        <TicketServiceDepartment hookFormProp={hookFormProp as PropForm}/>
        <Permissibles hookFormProp={hookFormProp as PropForm} />
